...
test("Gold AM (LBXAUAM): documented direct USD/oz value is preserved, not inverted", () => {
  ...
  assert.notEqual(result.normalized_price, 1 / 2650.45);
});
...
test("parseProviderResponse: partial failure does not discard valid symbols in the same batch", () => {
  ...
  assert.deepEqual(resultSymbols, [
    "LBXAUAM",
    "LME-XCU",
  ]);
  ...
  assert.deepEqual(failureSymbols, [
    "LBXAUPM",
    "LME-TIN",
  ]);

  const pmFailure = failures.find(
    (f) => f.symbol === "LBXAUPM"
  );

  assert.equal(pmFailure.code, "IMPLAUSIBLE_GOLD_PRICE");

  const tinFailure = failures.find(
    (f) => f.symbol === "LME-TIN"
  );
...
// ---------------------------------------------------------------------------
test("fetchMetalsPrices: throws a generic error when METALS_API_KEY is unset", async () => {
...
test("fetchMetalsPrices: request URL containing the key never appears in a thrown error message", async () => {
  const originalKey = process.env.METALS_API_KEY;

  process.env.METALS_API_KEY =
    "test-fake-key-do-not-leak-1234567890";
...
test("fetchMetalsPrices: end-to-end with a fake fetch returns normalized results for all four symbols", async () => {
  ...
  try {
    ...
    assert.equal(
      bySymbol.LBXAUAM.normalized_price,
      4301.85
    );

    assert.equal(
      bySymbol.LBXAUPM.normalized_price,
      4335.55
    );

    assert.equal(
      bySymbol.LBXAUAM.conversion_factor,
      null
    );

    assert.equal(
      bySymbol.LBXAUPM.conversion_factor,
      null
    );

    assert.ok(
      Math.abs(
        bySymbol["LME-XCU"].normalized_price -
          0.4429 * 32150
      ) < 1e-9
    );
    ...
    assert.ok(
      Math.abs(
        bySymbol["LME-TIN"].normalized_price -
          1.7449 * 32150
      ) < 1e-9
    );
  } finally {
